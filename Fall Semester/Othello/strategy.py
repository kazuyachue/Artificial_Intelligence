...
def placeToken(b, t, p):
    ...
    for neighbors in lookupTable[p]:
        if b[neighbors] == opponent:
            if isSandwich(b, you, neighbors, p):
                diff = neighbors - p
                location = p
                while 1:
...
def evalBoard(board, token):
    enemy = 'O' if token == 'X' else 'X'
    if board.count('.') < 14: #endgame
        return board.count(token) - board.count(enemy)
    else:
        weighting_matrix = [
        240, -40,  20,   5,  5,  20, -40, 240,
        -40, -80,  -5,  -5,  -5,  -5, -80, -40,
        20,  -5,  15,   3,   3,  15,  -5,  20,
        5,  -5,   3,   3,   3,   3,  -5,   5,
        5,  -5,   3,   3,   3,   3,  -5,   5,
        ...
        for i in [0, 7, 56, 63]:
            if board[i] == token:
                for neighbor in lookupTable[i]:
                    weighting_matrix[neighbor] = abs(weighting_matrix[neighbor])
        matrix = [1 if i == token else -1 if i == enemy else 0 for i in board]
        heuristic = sum(weighting_matrix[n] * matrix[n] for n in range(len(weighting_matrix)))
        mobility = len(possibleMoves(board, token)) - len(possibleMoves(board, enemy))

        return heuristic + 8 * mobility + random.randint(-1, 1) #random noise
